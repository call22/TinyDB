    ...
    public Result execute(Manager manager) throws RuntimeException{
        ...
        switch (this.type){
            ...
            case show_db:{
                ...
                result = new Result();
                // 【数据库名称 | 包含的表单数】
                Column dbName = new Column("Database Name", ColumnType.STRING, true, true, 255);
                Column tableNum = new Column("Table Number", ColumnType.INT, false, true, 0);
            ...
            case show_table:{
                // 给定databaseName
                Database db = manager.selectDatabase(databaseName);
                if(db != null) {
                    LinkedList<Table> tables = db.getTables();
                    result = new Result();
                    // 【表单名称 | 包含属性数】
                    Column tableName = new Column("Table Name", ColumnType.STRING, true, true, 255);
                    Column columnNum = new Column("Column Number", ColumnType.INT, false, true, 0);
                    ...
                    result = Result.setMessage(this.databaseName + "doesn't exists");
                }
                break;
            }
            case show_meta:{
                ...
                result = new Result();
                // 【属性名 | 类型 | 主键 | 是否为空 | 最大长度】
                Column columnName = new Column("Column Name", ColumnType.STRING, true, true, 255);
                Column type = new Column("Type", ColumnType.STRING, false, true, 7);
                Column primary = new Column("Is Primary", ColumnType.INT, false, true, 0);
                Column isNull = new Column("Is Null", ColumnType.INT, false, true, 0);
                Column maxLength = new Column("Max Length", ColumnType.INT, false, true, 0);
